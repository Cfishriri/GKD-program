...
data_path=data/gsm8k_frameworks_v3.jsonl
generation_audit_path=data/gsm8k_frameworks_v3.generation-audit.json
purity_audit_path=data/gsm8k_frameworks_v3.audit.json
partial_path=${data_path}.partial

if [[ -e "$data_path" || -e "$partial_path" || -e "$generation_audit_path" || -e "$purity_audit_path" ]]; then
  echo "Refusing to overwrite existing v3 framework data or audit files." >&2
  echo "Archive the existing v3 files before starting a new generation run." >&2
  exit 1
fi
